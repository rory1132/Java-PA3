...
public class ReplaySokobanGame extends AbstractSokobanGame {
            ...
            if (mode==Mode.ROUND_ROBIN) {
                ...
                var copy = index;
                    ...
                    while (!shouldStop() || !firstExit) {
                        ...
                        while ((abc.currentIndex % colIndex) != copy&&count!=(colIndex-1)) {
                            try {
                                Thread.sleep(30);
                            } catch (InterruptedException e) {
                                throw new RuntimeException(e);
    ...
    public void run() {
        // TODO
        if (mode==Mode.ROUND_ROBIN) {
            colIndex = inputEngines.size();
            abc = new ABC();
        }

        Thread[] ths = new Thread[inputEngines.size()+1];
        for (int i = 0; i < inputEngines.size(); i++) {
            ths[i+1] = new Thread(new InputEngineRunnable(i,inputEngines.get(i)));
        }
        ths[0] = new Thread(new RenderingEngineRunnable());
